...
LUA_SHA1 = get_hash(os.path.join(CWD, 'ua.lua'))
...
class WgetArgs(object):
    ...
    def realize(self, item):
        ...
        item_name = []
        for url in item['item_name'].split('\0'):
            temp_url = url if url.count('/') > 2 else url + '/'
            for prob, pattern in self.patterns():
                if re.search(pattern, temp_url):
                    if random.random() < prob:
                        item_name.append(url)
                    break
        item['item_name'] = '\0'.join(item_name)
...
project = Project(
    title = '.ua',
    project_html = '''
    <img class="project-logo" alt="logo" src="https://wiki.archiveteam.org/images/0/04/Ua-cctld-logo.png" height="50px"/>
    <h2>Archiving the .ua space. &middot; <a href="http://tracker.archiveteam.org/ua/">Leaderboard</a></span></h2>
    '''
)
